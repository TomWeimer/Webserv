...
void ManageConnection::create_new_client(int socketFd)
{
	...
	Server	&server = _register.find_server(socketFd);
	ClientSocket new_socket(client);
	add_socket(server, new_socket, _all_sockets);
}
...
Request ManageConnection::receive_request(ClientSocket *socket, Server *server)
{
	Request new_request;
	int		buffer_size;
...
{
	int sockfd = new_socket.sockfd();
	if (new_socket.sockfd() > _max_socket)
		_max_socket = sockfd;
	_register.add_entry(sockfd, new_socket);
	_register.add_entry(sockfd, server);
	socket_set.add_socket(new_socket);
